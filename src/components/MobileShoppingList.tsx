...
function Row({ item }: { item: ShoppingItem }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <div
          className={`w-7 h-7 rounded-sm border-2 flex items-center justify-center shrink-0 ${
            item.checked ? "bg-zinc-900 border-zinc-900 text-white" : "border-zinc-400 bg-white"
          }`}
        >
          {item.checked ? (
            <svg viewBox="0 0 20 20" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="3">
